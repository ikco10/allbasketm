...
import com.ikco10.allbasketsignup.R;
...
public class FastScroller {
    ...
    public FastScroller(@NonNull ViewGroup view, @NonNull ViewHelper viewHelper,
        ...
        mMinTouchTargetSize = view.getResources().getDimensionPixelSize(
                R.dimen.fastscroller_min_touch_target_size);
    ...
    */
    ...
    private boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        if (!mDragging) {
            touchposition = (int) (eventY + padding.top);
            thumbtop = (int) (mThumbOffset + padding.top - mThumbHeight / 5f);
            thumbbottom = (int) (mThumbOffset + padding.top + mThumbHeight * 1.6f);
            thumbstart = mView.getWidth() - mThumbWidth - padding.right;
        }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (thumbbottom > touchposition && touchposition > thumbtop && eventX > thumbstart) {
    ...
    private boolean isInView(@NonNull View view, float x, float y) {
        int scrollX = mView.getScrollX();
        int scrollY = mView.getScrollY();
        return x - mMinTouchTargetSize >= view.getLeft() - scrollX && x - mMinTouchTargetSize < view.getRight() - scrollX
    ...
    private boolean isInViewTouchTarget(@NonNull View view, float x, float y) {
        int scrollX = mView.getScrollX();
        int scrollY = mView.getScrollY();
        return isInTouchTarget(x - mMinTouchTargetSize, view.getLeft() - scrollX, view.getRight() - scrollX,
                mView.getWidth())
                && isInTouchTarget(y, view.getTop() - scrollY, view.getBottom() - scrollY,
                mView.getHeight());
    }

    private boolean isInTouchTarget(float position, int viewStart, int viewEnd,
                                    int parentEnd) {
